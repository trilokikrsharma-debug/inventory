...
<div class="no-print print-bar">
    <button id="btnPrintShared" class="print-bar-btn print-bar-btn-primary">
        <?= Helper::escape($printLabel) ?>
    </button>
    <?php if ($docId > 0): ?>
    <a href="<?= APP_URL ?>/index.php?page=invoice&action=download&type=<?= urlencode($docType) ?>&id=<?= $docId ?>" class="print-bar-link">
        Download PDF
    </a>
    <?php endif; ?>
    <button id="btnCloseShared" class="print-bar-btn print-bar-btn-secondary">
        Close
    </button>
</div>
