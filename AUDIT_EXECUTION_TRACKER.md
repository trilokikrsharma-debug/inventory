...
- [ ] Add local first-party QR rendering for 2FA setup without reintroducing third-party secret disclosure
...
- 2026-04-13: Performed a focused security audit of auth/import flows and fixed four concrete issues. `core/ApiAuth.php` now fails closed on empty/invalid scope sets instead of silently creating full-access tokens; `services/ContactImportService.php` and `services/ProductImportService.php` now scope import lookups by `Tenant::id()` to stop cross-tenant validation leakage; `controllers/TwoFactorController.php`, `services/TwoFactorService.php`, and `views/twoFactor/setup.php` no longer send TOTP secrets to Google Charts during 2FA setup, falling back to manual/deep-link provisioning until a local QR renderer is added. Added regression coverage in `tests/Unit/ApiControllerTest.php`, `tests/Unit/ContactImportServiceTest.php`, `tests/Unit/ProductImportServiceTest.php`, and `tests/Unit/TwoFactorServiceTest.php`, and synced the stale implementation note in `PHASE1_ENTERPRISE_FIXES.md`.
- 2026-04-13: Continued controller thinning in `controllers/ApiController.php` by extracting API token list loading, token-request normalization, expiry resolution, and tenant summary payload assembly into `services/ApiWorkflowService.php`. Added `tests/Unit/ApiWorkflowServiceTest.php`; the focused API/auth/import/2FA regression slice passes with `16` tests and `62` assertions.
- 2026-04-13: Continued controller thinning in `controllers/TwoFactorController.php` by extracting 2FA setup payload assembly, post-2FA login-context rebuilding, and session-user sanitization into `services/TwoFactorWorkflowService.php`. Added `tests/Unit/TwoFactorWorkflowServiceTest.php`; the focused auth/import/2FA regression slice passes with `19` tests and `76` assertions.
- 2026-04-13: Continued controller thinning in `controllers/HrController.php` by extracting the HR index/dashboard aggregation path into `services/HrWorkflowService.php`, including employee search/stats loading, attendance and leave summaries, upcoming holidays, shift counting, and payroll snapshot assembly. Added `tests/Unit/HrWorkflowServiceTest.php`; the focused HR/auth/import/2FA regression slice passes with `23` tests and `97` assertions.
