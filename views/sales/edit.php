...
<div class="page-header">
    <nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="<?= APP_URL ?>">Dashboard</a></li><li class="breadcrumb-item"><a href="<?= APP_URL ?>/index.php?page=sales">Sales</a></li><li class="breadcrumb-item active">Edit <?= Helper::escape($sale['invoice_number']) ?></li></ol></nav>
    <div class="app-page-actions">
        <a href="<?= APP_URL ?>/index.php?page=sales&action=view_sale&id=<?= $sale['id'] ?>" class="btn btn-outline-secondary"><i class="fas fa-arrow-left me-1"></i>Back to Sale</a>
    </div>
</div>
                ...
                <div class="card-body"><div class="row g-3 sales-entry-details-grid">
                    ...
                    <div class="col-md-6 col-xl-4"><label class="form-label">Warehouse <span class="text-danger">*</span></label>
                        <select name="warehouse_id" class="form-select" required><option value="">Select</option>
                            <?php foreach ($warehouses as $warehouse): ?><option value="<?= (int)$warehouse['id'] ?>" <?= (int)($sale['warehouse_id'] ?? 0) === (int)$warehouse['id'] ? 'selected' : '' ?>><?= Helper::escape($warehouse['name']) ?></option><?php endforeach; ?>
                        </select></div>
                    <?php endif; ?>
                    <div class="col-md-6 col-xl-4"><label class="form-label">Date <span class="text-danger">*</span></label><input type="date" name="sale_date" class="form-control" value="<?= $sale['sale_date'] ?>" required></div>
                    <div class="col-md-6 col-xl-4"><label class="form-label">Reference</label><input type="text" name="reference_number" class="form-control" value="<?= Helper::escape($sale['reference_number'] ?? '') ?>"></div>
                </div></div>
            </div>
            <div class="card mb-3 sales-entry-card">
        ...
        <div class="col-lg-4">
            ...
            <div class="card mb-3 sales-entry-summary-card">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-2"><label class="form-label small">Discount</label><input type="number" name="discount_amount" class="form-control form-control-sm" step="0.01" value="<?= $sale['discount_amount'] ?>" id="discountInput" min="0"></div>
                    <?php if ((!isset($settings['enable_tax']) || $settings['enable_tax']) && (!isset($settings['enable_gst']) || $settings['enable_gst'])): ?>
                    <div class="mb-2">
            ...
            <div class="card mb-3 sales-entry-summary-card">
                ...
                <div class="card-body">
                    <div class="mb-2"><label class="form-label small">Paid</label><input type="number" name="paid_amount" class="form-control" step="0.01" value="<?= $sale['paid_amount'] ?>" id="paidInput" min="0"></div>
            ...
            <div class="sales-entry-summary-actions">
                <button type="submit" class="btn btn-warning btn-lg"><i class="fas fa-save me-2"></i>Update Sale</button>
                <a href="<?= APP_URL ?>/index.php?page=sales&action=view_sale&id=<?= $sale['id'] ?>" class="btn btn-outline-secondary">Cancel</a>
            </div>
            </div>
        </div>
    </div>
...
function addItem(prefill) {
    ...
    if (prefill) {
        ...
        row.querySelector('.tax').value = taxCalculationEnabled ? prefill.tax_rate : 0;
    }
    ...
    function updateSelection() {
        items.forEach((item, index) => {
            if (index === currentIndex) {
                item.style.background = 'var(--surface-soft, #f0f0f0)';
