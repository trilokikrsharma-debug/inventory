...
<div class="page-header">
    <nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="<?= APP_URL ?>">Dashboard</a></li><li class="breadcrumb-item"><a href="<?= APP_URL ?>/index.php?page=payments">Payments</a></li><li class="breadcrumb-item active"><?= $type === 'receipt' ? 'Receipt' : 'Payment' ?></li></ol></nav>
    <div class="detail-page-actions">
        <a href="<?= APP_URL ?>/index.php?page=payments" class="btn btn-outline-secondary btn-sm"><i class="fas fa-arrow-left me-1"></i>Back to Payments</a>
    </div>
</div>
    ...
    <div class="col-lg-8 col-xl-7">
        <div class="card detail-card">
            <div class="card-header">
                <h6><i class="fas fa-<?= $type==='receipt'?'hand-holding-usd':'paper-plane' ?> me-2"></i><?= $type==='receipt' ? 'Receive Payment from Customer' : 'Make Payment to Supplier' ?></h6>
                <span class="badge bg-<?= $type==='receipt' ? 'success' : 'primary' ?>"><?= ucfirst($type) ?></span>
            </div>
            <div class="card-body">
                <form method="POST"><?= CSRF::field() ?>
                    ...
                    <div class="mb-3"><label class="form-label">Customer <span class="text-danger">*</span></label>
                        ...
                        <div class="form-text">Select the customer account receiving this receipt.</div>
                    </div>
                    ...
                    <div class="mb-3"><label class="form-label">Supplier <span class="text-danger">*</span></label>
                        ...
                        <div class="form-text">Select the supplier account for this outgoing payment.</div>
                    </div>
                    <?php if (!empty($_GET['purchase_id'])): ?><input type="hidden" name="purchase_id" value="<?= (int)$_GET['purchase_id'] ?>"><?php endif; ?>
                    <?php endif; ?>
                    <div class="mb-3"><label class="form-label">Amount <span class="text-danger">*</span></label><input type="number" name="amount" class="form-control" step="0.01" required value="<?= !empty($_GET['amount']) ? (float)$_GET['amount'] : '' ?>"><div class="form-text">Enter the exact receipt or payment amount.</div></div>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6"><label class="form-label">Reference No.</label><input type="text" name="reference_number" class="form-control"></div>
                    ...
                    <div class="detail-page-actions">
                        <a href="<?= APP_URL ?>/index.php?page=payments" class="btn btn-outline-secondary">Cancel</a>
                        <button type="submit" class="btn btn-<?= $type==='receipt'?'success':'primary' ?>"><i class="fas fa-save me-2"></i>Save <?= ucfirst($type) ?></button>
                    </div>
                </form>
            </div>
        </div>
